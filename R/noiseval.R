...
#'
#' This function allows you to inject windows of white noise into a time series. This is meant to mimic a broken sensor that is producing random values from a normal distribution for a specified period of time.
...
#' @param noise.sd Random noise is pulled for each error event from a normal distribution. noise.sd is the standard deviation of that distribution. There is no default value.
...
#' d1 <- simseries
#' str(d1)
#' noiseval(d1$v1, n=5, min.dur=100, max.dur=1000, noise.sd=1, plot=T)
#' d2 <- noiseval(d1$v1, n=5, min.dur=100, max.dur=1000, noise.sd=1, plot=T)
#' str(d2)

noiseval <- function(y, n=1, min.dur=1, max.dur=1, 
                      noise.sd, plot=F) {
  ### assign objects
  original.data <- as.vector(y) # clean time series
  dat <- y
  tvec <- 1:length(y) # vector with length of y
  nnoise <- n # number of stuck events
  sdnoise <- noise.sd # sd of osc amplitude
  minnoise <- min.dur # minimum duration of osc events
  maxnoise <- max.dur # maximum duration of osc events
  plot <- plot # plot option
  ### create indicator vector
  noise_ind <- as.vector(rep(0, length(y)))
  ### input osc values
  for(i in 1:nnoise){
    duranom <- sample(1:(maxnoise-minnoise), size=1)
    tstart <- sample(tvec, size=1)
    tend <- tstart + duranom
    dat[tstart:tend] <- rnorm(n=duranom, mean=dat[tstart], 
						sd=sdnoise)
    noise_ind[tstart:tend] <- as.vector(rep(1, duranom))
  }
  ### prepare output list
  new.data <- as.vector(dat)
  error.indicator <- as.vector(noise_ind)
  ### plot option
  if(plot==T){
    par(mfrow=c(3,1))
